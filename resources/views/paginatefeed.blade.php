@foreach ($posts as $post)
  <div id="story-modal{{ $post->id }}b" class="uk-modal-container" uk-modal>
    <div class="uk-modal-dialog story-modal">
      <button class="uk-modal-close-default lg:-mt-9 lg:-mr-9 -mt-5 -mr-5 shadow-lg rounded-full p-4 transition"
          ...
          <div class="grid grid-cols-2">
            @foreach (unserialize(base64_decode($post->image)) as $imm)
              ...
              @else
                @if ($loop->index == 0)
                  <a href="{{ asset('/storage/' . $imm) }}" class="col-span-2">
                    <img src="{{ asset('/storage/' . $imm) }}" alt="" class="inset-0 h-full w-full object-cover"
                      style="height: 600px">
                  </a>
                @else
                  <a href="{{ asset('/storage/' . $imm) }}" class="col-span-2">
                    <img src="{{ asset('/storage/' . $imm) }}" alt=""
                      class="inset-0 h-full w-full object-cover hidden" style="height: 600px">
                  </a>
                @endif
              @endif
            @endforeach
            ...
            <ul class="space-y-1">
              ...
              <li>
                <form action="/post/{{ $post->id }}" method="POST"
                  ...
                  @method('delete')
                  @csrf
                  <button type="submit" style="display: inline-block" onclick="return confirm('Are you sure?')"><i
        ...
        @foreach (unserialize(base64_decode($post->image)) as $imm)
          ...
          @else
            @if ($loop->first)
              <a href="{{ asset('/storage/' . $imm) }}" class="col-span-2">
                <img src="{{ asset('/storage/' . $imm) }}" alt=""
                  class="rounded-md w-full lg:h-76 object-cover" style="height: 400px">
              </a>
            @elseif($loop->index == 1)
              <a href="{{ asset('/storage/' . $imm) }}">
